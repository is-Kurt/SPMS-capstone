...
-->
<div id="create-file-modal" class="hidden fixed inset-0 z-[150] flex items-center justify-center bg-zinc-950/40 backdrop-blur-sm transition-all">
    <div class="bg-surface rounded-2xl shadow-2xl w-full max-w-[450px] p-8 border border-surface-border">
                        ...
                        <select id="new-doc-template" name="template"
                            ...
                            <?php if(isset($templates) && !empty($templates)): ?>
                                <?php foreach($templates as $tpl): ?>
                                    <?php
                                        $label = match(strtoupper($tpl['title'])) {
                                            'IPCR' => 'IPCR — Individual Performance Commitment & Review',
                                            'DPCR' => 'DPCR — Division Performance Commitment & Review',
                                            'OPCR' => 'OPCR — Office Performance Commitment & Review',
                                            'IPERF' => 'IPERF — Job Order / COS Performance Rating',
                                            default => $tpl['title']
                                        };
                                    ?>
                                    <option value="<?= $tpl['id'] ?>" <?= strtoupper($tpl['title']) === 'IPCR' ? 'selected' : '' ?>><?= esc($label) ?></option>
                                <?php endforeach; ?>
                            <?php endif; ?>
