...
<main class="flex min-h-[calc(100vh-6rem)] flex-col justify-center px-6 py-12 lg:px-8">
    <div class="mx-auto w-full max-w-md border border-zinc-200 dark:border-zinc-800 p-10 rounded-2xl bg-white dark:bg-zinc-900 shadow-xl shadow-zinc-200/50 dark:shadow-none">
        ...
        <p class="text-center text-sm font-medium text-text-muted mb-6">Enter your current password to authorize changing your email address.</p>

        <?= form_open('profile/email/step1', ['class' => 'space-y-4']) ?>
            <div>
                <label for="password" class="block text-sm font-bold text-text">Current Password</label>
                <div class="mt-2">
                ...
                <div class="min-h-3 pl-1">
                    <p class="text-danger-500 text-[10px] font-bold mt-1 uppercase tracking-wider"><?= session('error') ?? validation_show_error('password') ?></p>
                </div>
            </div>
