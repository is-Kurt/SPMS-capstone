...
<div class="p-8 max-w-7xl mx-auto flex flex-col gap-6 h-[calc(100vh-6rem)]">
    ...
    <div class="flex items-center gap-4 shrink-0">
        ...
        <div>
            <h1 class="text-2xl font-black tracking-tight text-text"><?= esc($rating['title']) ?></h1>
            <p class="text-xs font-bold text-text-muted mt-1 uppercase tracking-widest">Evaluation Batch</p>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        badges.forEach(badge => {
            // Get the raw score from the HTML attribute
            const score = parseFloat(badge.getAttribute('data-score'));
            
            // 1. Call your existing function from functions.js
            const arLabel = getAdjectivalRating(score);
            badge.innerText = arLabel;
            
            // 2. Assign the correct Tailwind colors based on the label
            switch(arLabel) {
            ...
            // Fade the badge in smoothly
            badge.classList.remove('opacity-0');
        });
    });

    // Auto-save logic if the admin tries to close the tab while typing a remark
