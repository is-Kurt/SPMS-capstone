...
<div class="p-4 md:p-8 max-w-7xl mx-auto flex flex-col gap-6 md:gap-8 pb-20 lg:h-[calc(100vh-6rem)] lg:overflow-hidden">
    ...
    <div class="bg-surface lg:bg-surface/50 border-none lg:border border-surface-border rounded-none lg:rounded-2xl shadow-none lg:shadow-sm flex flex-col flex-1 lg:min-h-0 lg:overflow-hidden relative pb-10 lg:pb-0">
        ...
        <div id="tab-content-directory" class="tab-content flex flex-col lg:absolute lg:inset-0 bg-transparent lg:bg-surface">
            ...
            <div class="overflow-y-auto overflow-x-hidden custom-scrollbar flex-1 lg:bg-surface">
                <table class="w-full text-left border-collapse block lg:table">
                    ...
                    <tbody class="block lg:table-row-group divide-y lg:divide-y-0 divide-transparent lg:divide-surface-border" id="user-table-body">
                            ...
                            <tr class="user-dir-row block lg:table-row bg-surface border lg:border-none border-surface-border rounded-xl lg:rounded-none mb-3 lg:mb-0 p-4 lg:p-0 shadow-sm lg:shadow-none hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors <?= (isset($u['is_active']) && $u['is_active'] == 0) ? 'opacity-60 lg:opacity-60' : '' ?>"
                                    ...
                                    <div class="flex justify-between items-start lg:items-center">
                                        ...
                                        <div class="lg:hidden shrink-0 mt-0.5">
                                            <?php if (isset($u['is_active']) && $u['is_active'] == 1): ?>
                                                <span class="px-2 py-1 rounded-md text-[9px] font-black uppercase tracking-widest border bg-emerald-50 text-emerald-600 border-emerald-200">Active</span>
                                            <?php else: ?>
                                                <span class="px-2 py-1 rounded-md text-[9px] font-black uppercase tracking-widest border bg-red-50 text-red-600 border-red-200">Disabled</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="block lg:table-cell px-0 lg:px-6 py-2 lg:py-4 border-t border-surface-border lg:border-none mt-2 lg:mt-0">
                                    <div class="flex flex-col gap-2 lg:block">
                                        <span class="w-max px-3 py-1 rounded-lg text-[10px] font-black bg-blue-50 dark:bg-blue-500/10 text-blue-600 border border-blue-200 dark:border-blue-500/20 uppercase tracking-widest lg:w-auto">
                                ...
                                <td class="block lg:table-cell px-0 lg:px-6 pt-3 pb-0 lg:py-4 text-right border-t border-surface-border lg:border-none mt-2 lg:mt-0">
                                    <?php if ($u['id'] != session()->get('user_id')): ?>
                                        <div class="flex justify-end gap-2 lg:gap-2">
                                            <?= form_open('account/toggle', ['class' => 'inline flex-1 lg:flex-none']) ?>
                                                <input type="hidden" name="user_id" value="<?= $u['id'] ?>">
                                                <button type="submit" class="w-full lg:w-auto text-[10px] font-black uppercase tracking-widest px-4 py-2.5 lg:px-3 lg:py-1.5 rounded-lg border lg:border-transparent transition-colors cursor-pointer <?= $u['is_active'] == 1 ? 'border-amber-200 bg-amber-50 lg:bg-transparent text-amber-600 hover:bg-amber-100 lg:hover:bg-amber-50 dark:hover:bg-amber-500/10' : 'border-emerald-200 bg-emerald-50 lg:bg-transparent text-emerald-600 hover:bg-emerald-100 lg:hover:bg-emerald-50 dark:hover:bg-emerald-500/10' ?>">
                                                    <?= $u['is_active'] == 1 ? 'Disable' : 'Enable' ?>
                                                </button>
                                            <?= form_close() ?>

                                            <?= form_open('account', ['class' => 'inline flex-1 lg:flex-none', 'onsubmit' => "return confirm('Are you sure you want to permanently delete this user?');"]) ?>
                                                <input type="hidden" name="_method" value="DELETE">
                                                <input type="hidden" name="user_id" value="<?= $u['id'] ?>">
                                                <button type="submit" class="w-full lg:w-auto text-[10px] font-black uppercase tracking-widest px-4 py-2.5 lg:px-3 lg:py-1.5 rounded-lg border border-red-200 lg:border-transparent bg-red-50 lg:bg-transparent text-red-500 hover:bg-red-100 lg:hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors cursor-pointer">
                                                    Delete
                                                </button>
                                            <?= form_close() ?>
                                        </div>
                        ...
                        <tr id="empty-filter-state" class="hidden lg:table-row">
        ...
        <div id="tab-content-create" class="tab-content hidden flex-col lg:absolute lg:inset-0 overflow-y-auto custom-scrollbar bg-surface lg:bg-transparent rounded-xl border border-surface-border lg:border-none shadow-sm lg:shadow-none">
            <?= form_open('account/sendInvites', ['class' => 'p-4 md:p-8 flex flex-col gap-6 h-full']) ?>
                
                <div class="shrink-0">
                    <label class="block text-[10px] font-black uppercase tracking-widest text-text-muted mb-2">Assign Role</label>
                    <div class="relative w-full lg:max-w-md">
                        <select name="role_id" required class="appearance-none w-full bg-zinc-50 lg:bg-white dark:bg-zinc-900 border border-surface-border rounded-xl px-4 py-3 text-sm focus:border-accent outline-none text-text cursor-pointer font-bold">
        ...
        <div id="tab-content-system" class="tab-content hidden flex-col lg:absolute lg:inset-0 overflow-y-auto custom-scrollbar lg:bg-zinc-50/50 lg:dark:bg-zinc-900/30">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 p-0 lg:p-6 lg:h-full">
                ...
                <div class="bg-surface border border-surface-border rounded-2xl shadow-sm flex flex-col lg:h-full lg:overflow-hidden">
                            ...
                            <?php foreach ($roles as $role): ?>
                                <li class="flex justify-between items-center px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 rounded-lg transition-colors group">
                                    ...
                                    <?= form_open('account/role/delete', ['onsubmit' => "return confirm('Delete this role?');"]) ?>
                                        ...
                                        <button type="submit" class="text-red-400 hover:text-red-600 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity p-1">
                    ...
                    <?= form_open('account/role/add', ['class' => 'p-4 border-t border-surface-border bg-zinc-50 dark:bg-zinc-800/30 shrink-0']) ?>
                ...
                <div class="bg-surface border border-surface-border rounded-2xl shadow-sm flex flex-col lg:h-full lg:overflow-hidden">
                    ...
                    <?= form_open('account/position/add', ['class' => 'p-4 border-t border-surface-border bg-zinc-50 dark:bg-zinc-800/30 space-y-3 shrink-0']) ?>
                        ...
                        <div class="flex items-center justify-between">
                            <label class="flex items-center gap-2 cursor-pointer text-xs text-text-muted font-bold">
                                <input type="checkbox" name="is_teaching" value="1" class="w-4 h-4 rounded border-surface-border text-accent focus:ring-accent">
                                Is Teaching Staff?
                            </label>
                ...
                <div class="bg-surface border border-surface-border rounded-2xl shadow-sm flex flex-col lg:h-full lg:overflow-hidden">
                            ...
                            <?php foreach ($units as $unit): ?>
                                <li class="flex justify-between items-center px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 rounded-lg transition-colors group">
                                    <div class="flex flex-col pr-2">
                                        ...
                                        <?php else: ?>
                                            <span class="text-[9px] font-bold text-blue-500 uppercase tracking-widest mt-0.5">Top Level Node</span>
                                        <?php endif; ?>
                                    </div>
                    ...
                    <?= form_open('account/unit/add', ['class' => 'p-4 border-t border-surface-border bg-zinc-50 dark:bg-zinc-800/30 space-y-3 shrink-0']) ?>
                        ...
                        <div class="flex gap-2">
                            <select name="parent_id" class="flex-1 bg-white dark:bg-zinc-900 border border-surface-border rounded-lg px-2 py-2.5 lg:py-2 text-xs text-text outline-none cursor-pointer">
        ...
        function filterUsers() {
            ...
            rows.forEach(row => {
                ...
                if (matchesSearch && matchesRole && matchesDept && matchesStatus) {
                    // Mobile cards use block instead of table-row, rely on classes removing hidden state
            ...
            if (visibleCount === 0) {
                ...
                emptyState.classList.add('block', 'lg:table-row');
            } else {
                emptyState.classList.add('hidden');
                emptyState.classList.remove('block', 'lg:table-row');
            }
        }
